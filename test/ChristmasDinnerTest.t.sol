...
contract ChristmasDinnerTest is Test {
    ...
    function test_test_hostChange_recon() public {

    echidna.test_hostChange();
    ...
    function test_ether_lock() public{
        ...
        assert(address(cd).balance == 1e18);
        assert(deployer.balance == 0);
      //  vm.log(address(cd).balance);
        //vm.log(deployer.balance);

    }

      function echidna_ether_lock() public{
        vm.deal(user1, 1 ether);
        vm.prank(user1);
