...
contract ChristmasDinner {
    ...
    function deposit(address _token, uint256 _amount) external beforeDeadline {
        ...
        if(participant[msg.sender]){
            balances[msg.sender][_token] += _amount;// balance(host) = bal(host) + amount
            IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);//@audit - check CEI
            emit GenerousAdditionalContribution(msg.sender, _amount);
        } else {
            participant[msg.sender] = true;
            balances[msg.sender][_token] += _amount;//@audit - check CEI
     ...
     */
    function refund() external nonReentrant beforeDeadline { //Check this function again
        address payable _to = payable(msg.sender);
        _refundERC20(_to);
        _refundETH(_to);
        emit Refunded(msg.sender); //@audit-issue - Should have participation status too
    }
     ...
     */
    function changeParticipationStatus() external { //@audit-issue - logic contradicts with changehost
        if(participant[msg.sender]) {
     ...
     */
    function changeHost(address _newHost) external  {
        if(!participant[_newHost]) {
            revert OnlyParticipantsCanBeHost();
        }
        host = _newHost;// @audit - check on multiple hosts?
        emit NewHost(host);
    }
    ...
    function setDeadline(uint256 _days) external onlyHost {
        if(deadlineSet) {
            ...
            deadline = block.timestamp + _days * 1 days;//@audit-issue - set deadline to true
    ...
    function withdraw() external onlyHost {
        address _host = getHost();//@audit-issue - Missing ETh, as there were 4 valid tokens
        //@audit-issue - Host can steal funds before deadline
     ...
     */
    receive() external payable { //@audit-issue - no updation of participation status
        etherBalance[msg.sender] += msg.value; //@audit-issue - no deadline check
     ...
     */
    function _refundERC20(address _to) internal { //@audit-issue - No CEI
     ...
     * @param _to payable address passed from refund()
     */
    function _refundETH(address payable _to) internal { //@audit-issue - No CEI
